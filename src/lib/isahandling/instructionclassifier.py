...
class X86InstructionClassifier(InstructionClassifier):
    ...
    def _no_prefix(self, prefix: bytes) -> bool:
        for item in prefix:
            if item != 0:
                return False
        return True
    ...
    def classify(self, instruction_bytes: bytes) -> Any:
        ...
        if self._no_prefix(prefix) and (opcode[0] & 0xf0) == 0x70:
            return isa.X86Instructions.Jccrel8
        if self._no_prefix(prefix) and opcode[0] == 0xe0:
            return isa.X86Instructions.LOOPNErel8
        if self._no_prefix(prefix) and opcode[0] == 0xe1:
            return isa.X86Instructions.LOOPErel8
        if self._no_prefix(prefix) and opcode[0] == 0xe2:
            return isa.X86Instructions.LOOPrel8
        if self._no_prefix(prefix) and opcode[0] == 0xe3:
            return isa.X86Instructions.JECXZrel8
        if self._no_prefix(prefix) and opcode[0] == 0xe8:
            return isa.X86Instructions.CALLrel32
        if self._no_prefix(prefix) and opcode[0] == 0xe9:
            return isa.X86Instructions.JMPrel32
        if self._no_prefix(prefix) and opcode[0] == 0xeb:
            return isa.X86Instructions.JMPrel8
        # This might cause trouble
        if self._no_prefix(prefix) and opcode[0] == 0x0f and (opcode[1] & 0xf0) == 0x80:
            return isa.X86Instructions.Jccrel32
        if self._no_prefix(prefix) and opcode[0] in range(0xb0,0xb8):
        ...
        if self._no_prefix(prefix) and opcode[0] in range(0xb8, 0xc0):
            return isa.X86Instructions.MOVR32IMM32
        if self._no_prefix(prefix) and opcode[0] == 0x83 and modrm in range(0xc0, 0xc0+8):
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x81 and modrm in range(0xc0, 0xc0+8):
            return isa.X86Instructions.ADDR32IMM32
        if self._no_prefix(prefix) and opcode[0] == 0x83 and modrm in range(0xf0, 0xf0+8):
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x81 and modrm in range(0xf0, 0xf0+8):
            return isa.X86Instructions.XORR32IMM32
        if self._no_prefix(prefix) and opcode[0] == 0x83 and modrm in range(0xe8, 0xe8+8):
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x81 and modrm in range(0xe8, 0xe8+8):
            return isa.X86Instructions.SUBR32IMM32
        if self._no_prefix(prefix) and opcode[0] == 0x6a:
            return isa.X86Instructions.PUSHIMM8
        if self._no_prefix(prefix) and opcode[0] == 0x68:
            return isa.X86Instructions.PUSHIMM32
        if self._no_prefix(prefix) and opcode[0] in range(0x50, 0x50 + 8):
            return isa.X86Instructions.PUSHR32
        if self._no_prefix(prefix) and opcode[0] in range(0x58, 0x58 + 8):
            return isa.X86Instructions.POPR32
        if self._no_prefix(prefix) and opcode[0] == 0x89 and sib in range(0x20, 0x20 + 8) and (modrm & 0xC7) == 0x44:
            return isa.X86Instructions.MOVR32DISP8R32
        if self._no_prefix(prefix) and opcode[0] == 0x89 and sib in range(0x20, 0x20 + 8) and (modrm & 0xC7) == 0x84:
            return isa.X86Instructions.MOVR32DISP32R32
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000000) == 0b01000000 and (modrm & 0b00000111) != 0b100:
            return isa.X86Instructions.MOVR8DISP8MEM
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000000) == 0b10000000 and (modrm & 0b00000111) != 0b100:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000000) == 0b01000000 and (modrm & 0b00000111) != 0b100:
            return isa.X86Instructions.MOVR32DISP8MEM
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000000) == 0b10000000 and (modrm & 0b00000111) != 0b100:
            return isa.X86Instructions.MOVR32DISP32MEM
        if self._no_prefix(prefix) and opcode[0] == 0x8d and (modrm & 0b11000111) == 0b00000100:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000111) == 0b00000100 and (sib & 0b11000000) == 0:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000111) == 0b00000100 and (sib & 0b11000000) == 0:
            return isa.X86Instructions.MOVR32BI
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000111) == 0b00000100 and (sib & 0b11000000) != 0 and (sib & 0b00000111) != 0b101:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000111) == 0b00000100 and (sib & 0b11000000) != 0 and (sib & 0b00000111) != 0b101:
            return isa.X86Instructions.MOVR32BIS
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000111) == 0b00000100 and (sib & 0b00000111) == 0b101:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000111) == 0b00000100 and (sib & 0b00000111) == 0b101:
            return isa.X86Instructions.MOVR32IS
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000111) == 0b01000100:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000111) == 0b01000100:
            return isa.X86Instructions.MOVR32BISDISP8
        if self._no_prefix(prefix) and opcode[0] == 0x8a and (modrm & 0b11000111) == 0b10000100:
        ...
        if self._no_prefix(prefix) and opcode[0] == 0x8b and (modrm & 0b11000111) == 0b10000100:
